...
Ze względu na Fakt \ref{thm:jedoznacznosc-typu} możemy zakładać, że każdy \(\lambda\)-term ma w danym kontekście jednoznacznie określony typ.
...
\begin{definicja}\label{def:stopien-termu}(stopień \(\lambda\)-termu)\\
  Stopniem \(\mathrm{d}(M)\) \(\lambda\)-termu \(M\) nazywamy supremum stopni \(\beta\)-redekstów, które są w nim zawarte,
