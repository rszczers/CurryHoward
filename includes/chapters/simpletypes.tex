...
W dalszym ciągu będziemy chcieli utożsamić ze sobą wszystkie te pseudo-pretermy, które różnią się między sobią tylko zmiennymi związanymi tak, jak to ma miejsce w na przykład przypadku \(\lambda x.\,x\) i \(\lambda y.\,y\).
...
które różnią się tylko zmienną związaną.
...
%\end{definicja}
%\begin{definicja}(Asercja)\\
%  \emph{Asercją} (ang. \emph{judgement}) nazywamy każdą trójkę \((\Gamma ,\,M,\,\sigma )\in\mathcal{P}(\mathrm{V}\times \mathbf{T_\to})\times\mathbf{\Lambda}_{\mathrm{T}}\times\mathbf{T_\to}\), gdzie \(\Gamma\) jest otoczeniem typowym i oznaczamy \(\Gamma\vdash M^{\sigma}\).
%
