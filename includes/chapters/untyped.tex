...
\subsection{Język rachunku \(\lambda\)}
...
\subsection{Sematyka operacyjna}
\subsubsection{Redukcje i ich własności}
...
Przez \emph{domknięcie relacji} \(\mathcal{R}_1\) względem własności \(P\) będziemy rozumieli najmniejszą (w sensie mnogościowym) relację \(\mathcal{R}_2\) taką, że \(\mathcal{R}_1\subset\mathcal{R}_2\) i \(\mathcal{R}_2\) ma własność \(P\).
  ...
  \begin{enumerate}[label=(\alph*), ref={(\alph*)}]
    ...
    \item Przez \(\mathcal{R}^{*}\) oznaczamy zwrotne domknięcie relacji \(\mathcal{R}^{+}\).\label{def:closure_b}
...
\begin{definicja}(Strategia redukcji)
  Strategią redukcji nazywamy każde odwzorowanie \(S:\:\mathbf{\Lambda}\to\mathbf{\Lambda}\), które spełnia następującą własność
  \[
     S(M) = \begin{cases}
