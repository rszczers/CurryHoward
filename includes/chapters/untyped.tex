...
\begin{wniosek}
  ...
  \begin{align*}
    ...
    \item  Jeśli \(M=_{\alpha} N\) w konsekwencji \ref{def:alpha_3} dla \(M\equiv M'Z\) i \(N\equiv N'Z\) takich, że \(M'=_{\alpha}N'\), to z założenia indukcyjnego oczywiście \(N'=_{\alpha}M'\), a zatem z \ref{def:alpha_3} \(N=_{\alpha} M\).\label{proof:alpha_cases_4}
...
\begin{definicja}[Postać HNF, WHNF]
  ...
  \begin{enumerate}
    ...
    \item \emph{słabej czołowej postaci normalnej} (ang. \emph{weak head normal form}), jeśli
      \[M\equiv F\, P_1\, P_2\,\dots\,P_n\quad \text{dla}\ n\geq 0,\] gdzie 
      \(F\) jest albo \(\lambda\)-zmienną, albo \(\lambda\)-abstraktorem wiążącym zmienną, czyli wyrażeniem postaci  
...
\begin{przyklad}
  \(\lambda x.(\lambda y.\,y)\,N\) jest w w postaci WHNF, ale nie jest w postaci HNF, ponieważ zawiera redeks czołowy \((\lambda y.\,y)N\).
\end{przyklad}
...
\begin{konwencja*}
  ...
  \end{align*}
  Jest to przykład tzw. \emph{cukru syntaktycznego}, czyli wtórnych rozszerzeń języka, które ułatwiają jego użycie. Wyrażenia let w których \(M\equiv\lambda y.M'\) dla pewnego \(M'\in\mathbf{\Lambda}\) nazywamy \emph{domknięciami}\footnote{Idiom ten w literaturze poświęconej językom programowania z rodziny Lisp powszechnie występuje pod nazwą wyrażeń \emph{let-over-lambda}.} (ang. \emph{closure}). Nieformalnie, pozwalają one na przypisywanie wartości zmiennym o tzw. \emph{zakresie leksykalnym}.
\end{konwencja*}
