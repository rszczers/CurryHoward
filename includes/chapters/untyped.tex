...
\begin{definicja}(\(\beta\)-redukcja)
  ...
  \]
  \emph{\(\beta\)-redeksami} bedziemy nazywali wyrażenia postaci \((\lambda x.\, M)N\), zaś rezultat ich \(\beta\)-redukcji w postaci termu \(M[x/N]\) -- \emph{\(\beta\)-reduktem}. 
...
\end{definicja}
  \begin{przyklad}
    \begin{enumerate}[label={(\alph*)}, ref={(\alph*)}]
    \setlength\itemsep{0em} 
    \item Oznaczmy \(\mathrm{Y}\equiv\lambda f.\,(\lambda x.\,(f(xx))\lambda x.\,(f(xx)))\) i niech \(F\) będzie dowolnym \(\lambda\)-termem. Wówczas otrzymujemy nieskończony ciąg redukcji postaci
    \begin{align*}
    ...
    \end{align*}
      ...
      \item Niech \(\Omega\equiv (\lambda x.\ xx)(\lambda x.\ xx)\). \(\Omega\) jest \(\beta\)-redeksem, którego redukcja prowadzi do ponownego otrzymania termu \(\Omega\) i w konsekwencji do stałego ciągu redukcji postaci:
      \begin{align*}
