...
\begin{align*}
  \mathrm{foldl}  &\equiv \lambda f\,x\,n.\ n\,x\,(\lambda n.\ (foldl\ f\ x\ n))
\end{align*}
...
Przedstawimy teraz klasyczny sposób kodowania typów po raz pierwszy zaprezentowany dla liczb naturalnych przez A. Churcha w \cite{Church1941-CHUTCO-7}. Różni się on od kodowania Scotta tylko w przypadku typów rekursywnych, w pozostałych przypadkach obydwa kodowania dają te same rezultaty.  W kodowaniu Churcha w następujący sposób reprezentujemy typ \texttt{Nat}:
\begin{align*}
...
\begin{align*}
  \bar{1} &\equiv \mathrm{Succ\ Zero} \ =_\beta\ \lambda f x.\, fx\\
  \bar{2} &\equiv \mathrm{Succ\ Succ\ Zero}\ =_\beta\ \lambda f x.\, f\,f\,x\\
    &\vdots \\
  \bar{n} &\equiv \mathrm{Succ}^n\ \mathrm{Zero}\  =_\beta\  \lambda f x.\, f^n\,x
\end{align*}
...
Porównując je do kodowania Scotta widzimy, że różnica polega na aplikowaniu do kontynuacji termu \((n\,f\,x)\) w przypadku brania następnika, podczas gdy w przypadku kodowania Scotta aplikowaliśmy tylko pierwszy z argumentów. Zauważmy, że w kodowaniu Churcha mamy wobec tego:
\begin{align*}
  \bar{0} &=_\beta\ \mathrm{Zero}_{Ch}\\
  \bar{1} &=_\beta\ \mathrm{foldl\ Succ_{Ch}\ Zero_{Ch}\ \bar{0}}\\
  \bar{2} &=_\beta\ \mathrm{foldl\ Succ_{Ch}\ Zero_{Ch}\ \bar{1}}\\
    &\vdots \\
  \overline{n} &=_\beta\ \mathrm{foldl\ Succ_{Ch}\ Zero_{Ch}\ \overline{n-1}}
\end{align*}
Szerszą dyskusję na temat operacji arytmetycznych jakie można wprowadzić na tak zdefiniowanych obiektach można prześledzić w \cite{hinze_2005}.
