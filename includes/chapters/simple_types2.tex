...
\begin{definicja}(Typowalność)\\
  ...
  \end{enumerate}
  Tak określony obiekt będziemy nazywali \emph{wyprowadzeniem} typu i pisali \(\Gamma \vdash_\mathbb{T} M:\sigma\). O stwierdzeniu \(\Gamma \vdash M:\sigma\) będziemy mówili, że jest \emph{wyprowadzalne}. %Jeśli wyprowadzalność stwierdzenia będzie jednoznacznie wynikała z kontekstu, to będziemy omijać indeks \(\mathbb{T}\) przy symbolu \(\vdash_\mathbb{T}\).
\end{definicja}
...
\end{lemat}
\begin{dowod}
  Załóżmy, że stwierdzenie \(J: \Gamma\vdash M:\sigma\) jest wyprowadzalne. Dowód przebiega przez indukcję wględem długosci wyprowadzenia \(J\). Rozważmy następujące przypadki:
...
\end{lemat}
\begin{dowod}
  Prosty dowód przeprowadzamy przez indukcję względem długości wyprowadzenia stwierdzenia \(J\). Rozważmy następujące przypadki:
  \begin{enumerate}[label=(\alph*)]
    \setlength\itemsep{0em}
