...
\begin{definicja}(Poprawność)
  \(\lambda\)-term \(M\in\mathbf{\Lambda}\) nazywamy \emph{poprawnym} (ang. \emph{legal}), jeśli istnieje kontekst \(\Gamma\) i typ \(\rho\in\mathbb{T}\) taki, że istnieje wyprowadzenie \(\Gamma\vdash M:\rho\).
\end{definicja}
...
\begin{lemat}
  ...
  \begin{enumerate}[label=(\arabic*)]
    \setlength\itemsep{0em}
    \item \(\llbracket \tau \rrbracket  \subseteq \mathrm{SN}\).
    \item Jeśli \(N_1,\,N_2,\,\dots,\,N_k\in\mathrm{SN}\), to \(xN_1 N_2 \dots N_k \in \llbracket \tau \rrbracket\).
  \end{enumerate}
\end{lemat}
\begin{dowod}
  \qed
