...
\begin{definicja}\label{def:f_subterm}(Multizbiór \(\mathrm{Sub}\) podtermów pretermu)
  ...
  \begin{enumerate}[label={(\arabic*)}, ref={\arabic*}]
    ...
    \item \(\mathrm{Sub}(\lambda x:\sigma.\, M) = \mathrm{Sub}(M)\cup \{\lambda x:\sigma.\, M\}\)\label{def:untyped_subterm_3}
...
%      Zauważmy, że nie istnieje \(\lambda\)-term typu \(\perp\). Typ, który nie ma inhabitantów nazywamy typem \emph{początkowym} (ang. \emph{initial}), \emph{zerowym} albo typem \emph{bottom}.
    ...
    \item Polimorficzne złożenie funkcji.
      ...
      \fa \begin{gathered} \lambda \tau \sigma:*.\,\lambda g: \tau \to \rho\  f:\sigma\to\tau\ x:\sigma.\ g(f a)\ :\\ \Pi\tau\sigma:*.(\tau\to\rho)\to(\sigma\to\tau)\to\sigma\to\rho\end{gathered} & (abs) 12\\ 
      \begin{gathered} \lambda \rho \tau \sigma:*.\,\lambda g: \tau \to \rho\  f:\sigma\to\tau\ x:\sigma.\ g(f a)\ :\\ \Pi\rho\tau\sigma:*.(\tau\to\rho)\to(\sigma\to\tau)\to\sigma\to\rho\end{gathered} & (abs) 13 
      \end{fitch}
      \end{equation*}
  \end{enumerate}
\end{przyklad}
...
\end{lemat}
\begin{dowod}
  Analogicznie do Lematu \ref{thm:subject_reduction}, dowód przeprowadzamy przez indukcję względem długości wyprowadzenia sądu \(\Gamma\vdash M;\tau\). Szczegóły pomijamy. \qed
\end{dowod}
