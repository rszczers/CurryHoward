...
%      Zauważmy, że nie istnieje \(\lambda\)-term typu \(\perp\). Typ, który nie ma inhabitantów nazywamy typem \emph{początkowym} (ang. \emph{initial}), \emph{zerowym} albo typem \emph{bottom}.
    \item Polimorficzna funkcja identycznościowa.\label{ex:f_typing_3}
      \begin{equation*}
      \begin{fitch}
        \fh \alpha : * & (form)\\
        \fa \fa x : \alpha & (var)\\
      ...
      \end{fitch}
      \end{equation*}
    \item Polimorficzna iteracja.\label{ex:f_typing_4}
      \begin{equation*}
      \begin{fitch}
        \fh \alpha : * & (form) \\
      ...
      \end{fitch}
      \end{equation*}
    \item Polimorficzne złożenie funkcji.\label{ex:f_typing_5}
      \begin{equation*}
      \begin{fitch}
        \fh \gamma: * & (form) \\
        \fa \fh \beta: * & (form) \\
...
\end{dowod}
  ...
  \begin{przyklad}(Liczebniki Churcha)
    ...
    \end{align}
    Ponownie aplikując (\ref{ex:f_church_3}) do \(\Gamma\vdash zero:nat\) mamy:
    \begin{align}
      \Gamma \vdash f (f x)\ \mathrm{nat}\ \mathrm{succ}\ \mathrm{zero}\ :\ \mathrm{nat} \tag{4}\label{ex:f_church_4}
    \end{align}

    Na podstawie lematu o redukcji podmiotu (Lemat \ref{thm:uniqueness_f}) widzimy, że każdy kolejny redukt poniższej redukcji ma typ nat. 
    \begin{align*}
